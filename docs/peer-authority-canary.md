# Split-layout peer-authority canary

This public `claritas-viz-test` canary contains synthetic test-only contract data. It does not copy or certify private Claritas product source. The existing `contracts/visualization-query` test remains intact; this second canary specifically exercises a split contract layout.
...
The immutable TJSV action compiles TypeSpec to generated JSON Schema B under `.typespec-json-schema-validator/`, then compares that generated witness with the independently authored JSON Schema A. Generated Schema B, parity reports, Contract IR, verification receipts and artifacts are evidence only; none is an editable authority.
